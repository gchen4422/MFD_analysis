...
for (eas_file in eas_files) {
  ...
  eas_sumstats_sig = fread(paste0("/scratch/negishi/chen4422/hapnest/multi-ans-sum-stats/MF_benchmarking/MFD_revision/summstats_to_finemap/", trait, "_EAS_GRCh38_combined_format_noMHC_to_finemap.csv")) %>% filter(PVAL < 5e-8)
  eur_sumstats_sig = fread(paste0("/scratch/negishi/chen4422/hapnest/multi-ans-sum-stats/MF_benchmarking/MFD_revision/summstats_to_finemap/", trait, "_EUR_GRCh38_combined_format_noMHC_to_finemap.csv")) %>% filter(PVAL < 5e-8)
  afr_sumstats_sig = fread(paste0("/scratch/negishi/chen4422/hapnest/multi-ans-sum-stats/MF_benchmarking/MFD_revision/summstats_to_finemap/", trait, "_AFR_GRCh38_combined_format_noMHC_to_finemap.csv")) %>% filter(PVAL < 5e-8)

  significant_snps = rbind(eas_sumstats_sig[,1:3],eur_sumstats_sig[,1:3],afr_sumstats_sig[,1:3])
  ...
  fwrite(
    merged_ranges_df,
    file = paste0("/scratch/negishi/chen4422/hapnest/multi-ans-sum-stats/MF_benchmarking/MFD_revision/summstats_to_finemap/", trait, "_risk_loci"),
