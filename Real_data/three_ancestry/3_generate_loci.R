...
#eas_files = eas_files[-2] # ANX does not have risk loci
#eur_files = eur_files[-2] # ANX does not have risk loci

#eas_files = eas_files[-c(1:6)]
...
for (eas_file in eas_files) {
  ...
  for(j in 1:nrow(candidate_region)){
    ...
    start = candidate_region[j,]$MinBP
    end = candidate_region[j,]$MaxBP
    loci_first = eur_ref[(eur_ref$V4 >= start) & (eur_ref$V4 <= end)]
    loci = loci_first[loci_first$V1 == candidate_region[j,]$CHR,]
    loci_snp = loci$V2
    ...
    loci_first = eas_ref[(eas_ref$V4 >= start) & (eas_ref$V4 <= end)]
    loci = loci_first[loci_first$V1 == candidate_region[j,]$CHR,]
    loci_snp = loci$V2
    dir.create(paste0("risk_loci_ld_eas_", trait))
    write.table(loci_snp,file = paste0(paste0("risk_loci_ld_eas_", trait,"/"),snplist), quote = FALSE, row.names=FALSE, col.names = F)
    ldname = paste0(paste0("risk_loci_ld_eas_", trait,"/loci_"), i)
    command = paste("~/gwas_software/plink2.0/plink2 --bfile ",paste0("/scratch/negishi/chen4422/UKBB_pheSVD-main/ukb_geno/in_sample_ld/1kg_build38/flipped_finemap_allbyall/lipids_v8/test_MF_ukb_tpmi/1kg_eas_maf_unrelated_biallelic_nodup_extracted_",trait,"_flipped")," --extract",paste0(paste0("risk_loci_ld_eas_", trait,"/"),snplist)," --allow-no-sex --make-bed --out",ldname)
    t1 <- system(command, intern = TRUE)

    eas_plink <- read.plink(paste0(ldname,".bed"))
    ...
    compute_cov(eas_mat, paste0(ldname,".ld"))


    #snp_to_exclude = c(names(unique_value_col_indices_eur),names(unique_value_col_indices_eas))
    #write.table(snp_to_exclude,file = paste0(paste0("risk_loci_ld_eas_", trait,"/"),snplist,"_snp_to_exclude"), quote = FALSE, row.names=FALSE, col.names = F)
    snp_to_exclude_eas = c(names(unique_value_col_indices_eas))
    write.table(snp_to_exclude_eas,file = paste0(paste0("risk_loci_ld_eas_", trait,"/"),snplist,"_snp_to_exclude"), quote = FALSE, row.names=FALSE, col.names = F)
...
for (eas_file in eas_files) {
  ...
  for(i in 1:nrow(candidate_region)){
    ...
    if(file.exists(paste0(ldname,".ld"))){
      message(paste("Skipping loci", i, "for trait", trait, "- already exists"))
      next
    }
