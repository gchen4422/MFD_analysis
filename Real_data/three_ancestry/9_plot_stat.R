...
region_cs<-res_all%>%group_by(Region) %>% summarise(across(c("CS","MESuSiE_cs", "SuSiE_cs","Paintor_cs","SuSiEx_cs","XMAP_cs","SuSiE_metal_cs"), ~ sum(.x, na.rm = TRUE)))
region_cs%>%summarise(sum(CS!=0),sum(MESuSiE_cs!=0),sum(SuSiE_cs!=0),sum(Paintor_cs!=0),sum(SuSiEx_cs!=0),sum(XMAP_cs !=0),sum(SuSiE_metal_cs !=0))
res_all %>%
  group_by(Region) %>%
  summarise(
    across(c("CS","MESuSiE_cs", "SuSiE_cs","Paintor_cs","SuSiEx_cs","XMAP_cs","SuSiE_metal_cs"),
...
MFD_median_z<-res_all%>%filter(CS==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
MESuSiE_median_z<-res_all%>%filter(MESuSiE_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
SuSiE_median_z<-res_all%>%filter(SuSiE_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
Paintor_median_z<-res_all%>%filter(Paintor_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
SuSiEx_median_z<-res_all%>%filter(SuSiEx_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
XMAP_median_z<-res_all%>%filter(XMAP_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)
SuSiE_metal_median_z<-res_all%>%filter(SuSiE_metal_cs==1)%>%summarise(zmax = median(pmax(abs(Z_eur),abs(Z_afr),abs(Z_eas),na.rm=T)))%>%pull(zmax)

#Paintor_20_median_z<-res_all%>%filter(Paintor_cs_sub_20==1)%>%summarise(zmax = median(pmax(abs(zscore_WB),abs(zscore_BB))))%>%pull(zmax)


cat(round(c(MFD_median_z,MESuSiE_median_z,SuSiE_median_z,Paintor_median_z,SuSiEx_median_z,XMAP_median_z,SuSiE_metal_median_z),2))
...
# Wilson score CI for enrichment ratio
compute_enrichment_stats <- function(a, K, c_bg, N_bg) {
  ...
  data.frame(enrichment = rr, ci_lower = ci_lo, ci_upper = ci_hi,
             ...
             sig_count = a, bg_count = c_bg, stringsAsFactors = FALSE)
}
...
) %>% mutate(Method = factor(Method, levels = c("MFD","MESuSiE","SuSiE", "Paintor","SuSiEx","XMAP","SuSiE_Meta")))%>%
...
# <fct>                <dbl>       <dbl>        <dbl>
#1 MFD               1.14        2.82         2.15
#2 MESuSiE           1.06        4.17         2.57
#3 SuSiE             1.04        2.86         2.09
#4 Paintor           1.70        2.80         2.10
#5 SuSiEx            1.42        3.44         2.38
#6 XMAP              1.24        3.46         2.35
#7 SuSiE_Meta        0          66.1          7.35

# Enrichment of 95% credible set without by PHENONAME (with CI)
calc_fold_enrichment_marginal_ci <- function(df, cs_col, ann_col_name) {
  active_df <- df %>%
    group_by(Region) %>%
    filter(sum(!!sym(cs_col)) != 0) %>%
    ungroup()
  cs_snps    <- active_df %>% filter(!!sym(cs_col) == 1)
  noncs_snps <- active_df %>% filter(!!sym(cs_col) == 0)
  K    <- nrow(cs_snps)
  N_bg <- nrow(noncs_snps)
  map_dfr(ann_col_name, function(ann) {
    a    <- sum(cs_snps[[ann]], na.rm = TRUE)
    c_bg <- sum(noncs_snps[[ann]], na.rm = TRUE)
    stats <- compute_enrichment_stats(a, K, c_bg, N_bg)
...
cs_methods <- list(
  list(col = "CS", method = "MFD"),
  list(col = "MESuSiE_cs", method = "MESuSiE"),
  list(col = "SuSiE_cs", method = "SuSiE"),
  list(col = "Paintor_cs", method = "Paintor"),
  list(col = "SuSiEx_cs", method = "SuSiEx"),
  list(col = "XMAP_cs", method = "XMAP"),
  #list(col = "CARMAX_cs", method = "CARMAx"),
  list(col = "SuSiE_metal_cs", method = "SuSiE_Meta")
)

CS_enrichment_long <- map_dfr(cs_methods, function(x) {
  res <- calc_fold_enrichment_marginal_ci(res_all, x$col, ann_col_name)
  res$Method <- x$method
  res
}) %>%
  mutate(
    Method = factor(Method, levels = c("MFD","MESuSiE","SuSiE","SuSiE_Meta","Paintor","SuSiEx","XMAP")),
    Cat = recode(annotation,
                 "non_synonymous_common" = "Non-synonymous",
                 "synonymous" = "Synonymous",
                 "promotor" = "Promotor",
                 "utr_comb" = "UTR",
                 "CRE" = "CRE",
                 "heart_ind_eQTL" = "Heart_eQTL",
                 "artery_ind_eQTL" = "Artery_eQTL",
                 "brain_ind_eQTL" = "Brain_eQTL"),
    Cat = factor(Cat, levels = c("Non-synonymous", "Synonymous", "UTR", "Promotor", "CRE", "Brain_eQTL", "Heart_eQTL", "Artery_eQTL")),
    Prop = round(enrichment, 2)
  )
...
#
################################################################################
# Use position_dodge (not dodge2) so error bars align with bars
...
p_signal <- ggplot(CS_enrichment_long, aes(x = Cat, y = Prop, fill = Method)) +
  ...
  scale_y_break(c(80, 200), scales = 0.3, space = 0.1) +
  theme_bw() +
  custom_theme() +
  theme(
...
pdf(paste0(plot_dir,"Supp_Enrichment_Comparison_Set_CS_updated.pdf"), width = 210/25.4, height = 120/25.4)
print(p_signal)
dev.off()
