...
for H2 in "${H2_VALUES[@]}"; do
  for CAUSAL in "${CAUSAL_VALUES[@]}"; do
    ...
    Rscript --verbose /scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb_external_ld/shared_50/4_shared_50_analysis_runsusiex.R "${GENE}" "${H2}" "${CAUSAL}"
  done
done
