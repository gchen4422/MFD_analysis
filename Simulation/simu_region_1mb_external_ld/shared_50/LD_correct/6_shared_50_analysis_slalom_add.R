...
# List of objects expected to be saved
objects_to_save <- c("susie_EU", "susie_BB", "MESuSiE_res", "xmap", "carmax_results")

# Check existence and print status
cat("\n--- Checking object existence before saving ---\n")
existence_check <- sapply(objects_to_save, exists)
print(existence_check)

# Optional: Stop execution if any are missing
if(!all(existence_check)){
  missing_objs <- names(existence_check)[!existence_check]
  stop(paste("Error: The following objects are missing:", paste(missing_objs, collapse=", ")))
} else {
  cat("All objects present. Proceeding to save.\n")
}
