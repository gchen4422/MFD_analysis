...
res_out<-"shared_50_external_ld_updated_xmap_meta_susie_rss.RData"
library(ggplot2)
library(ggrepel)
library(grid)
library(egg)
...
for(causal_num in 1:3){
  for(h2 in 1:2){
    ...
    data_dir<-paste0(wrk_dir,"summary_data/susie_rss/")
    ...
    set2 = which(paste0("CAUSAL_",causal_num,"_LOCI_",1:100,"_h2_",h2,"_susie_rss.mcmc.paintor")%in%list.files())
    ...
    for(LOCI_num in replicate_set){
      ...
      paintor_res<-fread(paste0(result_dir,"CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_susie_rss.mcmc.paintor"))%>%dplyr::rename(SNP = RSID)
      ...
      susie_weighted_cs<-rep(0,length(susie_weighted_susie_rss$pip))
      susie_weighted_cs[unlist(susie_weighted_susie_rss$sets$cs)]<-1
      
      SuSiE_weighted_res_dataframe<-data.frame(SNP = zfile_sub_susie_rss_weighted$RSID,SuSiE_Weighted_PIP = susie_weighted_susie_rss$pip,SuSiE_weighted_cs = susie_weighted_cs )
      
      zfile_complete <- zfile_complete %>%
        left_join(SuSiE_weighted_res_dataframe, by = "SNP")
      
      #format susie merged results
      susie_merged_cs<-rep(0,length(susie_merged_susie_rss$pip))
      susie_merged_cs[unlist(susie_merged_susie_rss$sets$cs)]<-1
      
      SuSiE_merged_res_dataframe<-data.frame(SNP = zfile_sub_susie_rss_merged$RSID,SuSiE_Merged_PIP = susie_merged_susie_rss$pip,SuSiE_merged_cs = susie_merged_cs )
      ...
      susiex_res_name  <- paste0(result_dir,"susiex_result/","SuSiEx_CAUSAL_",causal_num,
                                 "_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95_susie_rss.snp")
      susiex_cred_name <- paste0(result_dir,"susiex_result/","SuSiEx_CAUSAL_",causal_num,
                                 "_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95_susie_rss.cs")
      ...
      #format xmap results
      EU_cov<-as.matrix(fread(paste0(data_dir,"CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_susie_rss.LD1")))
      BB_cov<-as.matrix(fread(paste0(data_dir,"CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_susie_rss.LD2")))
