...
metal_results_loci = metal_results_loci_aligned %>% select(MarkerName,HetPVal,Z_final) %>% dplyr::rename(Z= Z_final)


metal_results_loci_sub = metal_results_loci %>% filter(MarkerName %in% zfile$RSID, HetPVal >0.05)
snps_keep <- metal_results_loci_sub$MarkerName
...
merged_bim_path <- paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb_external_ld/risk_loci_ld_merge/loci_1kg_", LD_BLOCK, ".bim")
...
merged_cov = as.matrix(fread(paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb_external_ld/risk_loci_ld_merge/loci_1kg_",LD_BLOCK,".ld")))  ## In-sample LD matrix after merge bfile of eur and afr
...
# subset all LD matrices to this SNP set, in this order
merged_cov_sub  <- merged_cov[snps_keep, snps_keep, drop = FALSE]
weighted_cov_sub <- weighted_cov[snps_keep, snps_keep, drop = FALSE]
##############################################
#
#		Run SuSiE
#
...
##########First case: Filter heteogenity variants############
susie_weighted_het<-susie_rss(metal_results_loci_sub$Z,weighted_cov_sub,check_prior=F, n = 600000)
susie_merged_het<-susie_rss(metal_results_loci_sub$Z,merged_cov_sub,check_prior=F, n = 600000)





##########Second case: susie rss correction for LD mismatch############

# Start with full data
Z_cur   <- metal_results_loci$Z
cov_cur_weighted <- weighted_cov
idx_cur <- seq_along(Z_cur)


all_outliers <- integer(0)

repeat {
  EU_diagnostic <- kriging_rss(Z_cur, cov_cur_weighted,n = median(zfile$N_1))
  ...
  cov_cur_weighted <- cov_cur_weighted[keep_idx, keep_idx, drop = FALSE]
  idx_cur          <- idx_cur[keep_idx]  # update original indices
  
}

zfile_sub_susie_rss_weighted <- zfile[idx_cur, ]
susie_weighted_susie_rss<-susie_rss(Z_cur,cov_cur_weighted,check_prior=F, n = 600000)


Z_cur   <- metal_results_loci$Z
cov_cur_merged <- merged_cov
idx_cur <- seq_along(Z_cur)

all_outliers <- integer(0)

repeat {
  EU_diagnostic <- kriging_rss(Z_cur, cov_cur_merged,n = median(zfile$N_1))
  outlier_index <- EU_diagnostic$plot$plot_env$idx
  ...
  Z_cur   <- Z_cur[keep_idx]
  cov_cur_merged <- cov_cur_merged[keep_idx, keep_idx, drop = FALSE]
  idx_cur          <- idx_cur[keep_idx]  # update original indices
  
}

zfile_sub_susie_rss_merged <- zfile[idx_cur, ]
susie_merged_susie_rss<-susie_rss(Z_cur,cov_cur_merged,check_prior=F, n = 600000)





##########Third case: slalom correction for LD mismatch############



summary_stat_slalom = fread(paste0(data_dir,"metal_slalom/CAUSAL_",num_causal,"_LOCI_",LD_BLOCK,"_h2_",h2_num,"_metal.slalom.txt"))
#summary_stat_slalom$Z  = metal_results_loci$Z 
slalom_index <- summary_stat_slalom %>%  mutate(is_outlier = r^2 > 0.6 & nlog10p_dentist_s > 4,is_outlier = coalesce(is_outlier, FALSE)  ) %>%  pull(is_outlier)


metal_results_loci_sub_slalom = metal_results_loci %>% filter(!slalom_index)
merged_cov_sub_slalom = merged_cov[!slalom_index,!slalom_index]
weighted_cov_sub_slalom = weighted_cov[!slalom_index,!slalom_index]


susie_weighted_slalom<-susie_rss(metal_results_loci_sub_slalom$Z,weighted_cov_sub_slalom,check_prior=F, n = 600000)
susie_merged_slalom<-susie_rss(metal_results_loci_sub_slalom$Z,merged_cov_sub_slalom,check_prior=F, n = 600000)



SuSiE_name<-paste0(result_dir,"Metal_SuSiE_CAUSAL_",num_causal,"_LOCI_",LD_BLOCK,"_h2_",h2_num,"_correction_updated.RData")


save(susie_weighted_het,susie_merged_het,susie_weighted_susie_rss,susie_merged_susie_rss,susie_weighted_slalom,susie_merged_slalom,
     zfile_sub_susie_rss_weighted,zfile_sub_susie_rss_merged,metal_results_loci_sub_slalom,file = SuSiE_name)
