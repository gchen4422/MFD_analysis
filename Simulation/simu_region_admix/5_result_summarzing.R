simulation_dir<-"/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_admix/"
...
res_out<-"admixed_population_sensitivity.RData"
library(ggplot2)
library(ggrepel)
library(grid)
library(egg)
...
for(causal_num in 1:3){
  for(h2 in 1){
    ...
    for(LOCI_num in replicate_set){
      ...
      # Align metal SuSiE results to paintor_res SNP set (METAL may drop SNPs during harmonization)
      if(exists("analysis_snps") && length(susie_weighted$pip) != nrow(paintor_res)){
        align_idx <- match(analysis_snps, paintor_res$SNP)
        # Remap PIPs (0 for SNPs not in meta-analysis)
        sw_pip <- rep(0, nrow(paintor_res)); sw_pip[align_idx] <- susie_weighted$pip; susie_weighted$pip <- sw_pip
        sm_pip <- rep(0, nrow(paintor_res)); sm_pip[align_idx] <- susie_merged$pip; susie_merged$pip <- sm_pip
        # Remap CS indices from analysis_snps positions to paintor_res positions
        susie_weighted$sets$cs <- lapply(susie_weighted$sets$cs, function(cs) align_idx[cs])
        susie_merged$sets$cs <- lapply(susie_merged$sets$cs, function(cs) align_idx[cs])
        cat(" [aligned metal SuSiE:", length(analysis_snps), "->", nrow(paintor_res), "SNPs] ")
      }
      ...
      ld_dir = paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_admix/causal_num_",causal_num,"/")
      ...
      multisusie_dir = "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_admix/multi_susie_result/"
      ...
      # Use intersect of both ancestries CS (all CS via unlist)
      carmax_cs <- tryCatch({
...
PIP_calibration_Shared<-PIP_calibration_Shared%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Shared","SuSiE" = "SuSiE_Shared","SuSiE_meta_weighted" = "SuSiE_weighted_PIP","SuSiE_meta_merged" = "SuSiE_merged_PIP","Paintor" = "Paintor_PIP","SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP","CARMAX" = "CARMAX_Shared"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","SuSiE_meta_weighted","SuSiE_meta_merged","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX"))
...
res_dir_table <- "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_admix/res_summary/supp_tables/"
