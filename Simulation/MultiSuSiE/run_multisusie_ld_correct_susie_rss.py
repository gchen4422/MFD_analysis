        ...
        for flip_prop in flip_props:
            for LD_BLOCK in LD_BLOCK_range:
                for h2_num in h2_num_range:
                    ...
                    # Names matching the R paste0 convention
                    ...
                    result_dir = os.path.join(wrk_dir, f"result/flipped_{flip_prop}_rho_{rho_num}_susie_rss") + "/"
                    os.makedirs(os.path.join(result_dir, "multisusiex_result"), exist_ok=True)

                    # ----- Read zfile (structured array) -----
                    zfile_path = f"{data_dir_flipped}CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{h2_num}_flipped_{flip_prop}_rho_{rho_num}_susie_rss"
                    zfile_path_ori = f"{data_dir_flipped}CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{h2_num}_flipped_{flip_prop}_rho_{rho_num}"
                    ...
                    try:
                        EU_cov = np.genfromtxt(f"{data_dir_flipped}CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{h2_num}_flipped_{flip_prop}_rho_{rho_num}_susie_rss.LD1",delimiter=" ")
                        BB_cov = np.genfromtxt(f"{data_dir_flipped}CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{h2_num}_flipped_{flip_prop}_rho_{rho_num}_susie_rss.LD2",delimiter=" ")
                    except Exception as e:
                    ...
                    print("same length:", ori_subset_rsid.size == new_rsid_arr.size)
                    print("same order:", np.array_equal(ori_subset_rsid, new_rsid_arr))
                    ...
                    # filter out missing rows so MAFs match zfile
