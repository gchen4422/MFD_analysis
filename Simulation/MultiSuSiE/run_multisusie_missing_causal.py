            ...
            for External_index in External_indexs:
                for LD_BLOCK in LD_BLOCK_range:
                    for h2_num in h2_num_range:
                        ...
                        zfile_path = f"{data_dir}Missing_CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{int(h2_num)}"
                        try:
                            zfile = np.genfromtxt(
                                zfile_path,
                                delimiter=" ",
                        ...
                        # ----- Missingness filter (R logic) -----
                        if causal_index == 1:
                            # drop if missing in EU or BB
                            missing_mask = (zfile["EU_missing"] != 0) | (zfile["BB_missing"] != 0)
                        elif causal_index == 2:
                            # drop if missing in EU
                        ...
                        if keep_idx.size == 0:
                            print(
                                f"[missingness] All SNPs filtered out: "
                        ...
                        try:
                            EU_cov = np.genfromtxt(
                                f"{data_dir}Missing_CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{int(h2_num)}.LD1",
                                delimiter=" "
                            )
                            BB_cov = np.genfromtxt(
                                f"{data_dir}Missing_CAUSAL_{num_causal}_LOCI_{LD_BLOCK}_h2_{int(h2_num)}.LD2",
                                delimiter=" "
                            )
                        except Exception as e:
                        ...
                        if EU_cov.shape != (m, m) or BB_cov.shape != (m, m):
                            print(
                                f"[LD] shape mismatch (expected {m}x{m}): "
                                f"EU={EU_cov.shape}, BB={BB_cov.shape}"
                            )
                            continue

                        # Subset LD by kept SNPs
                        ...
                        try:
                            maf_EU = np.loadtxt(
                                "/scratch/negishi/chen4422/hapnest/Simulation_update/"
                                f"simu_region_1mb/risk_loci_ld_eur/maf_loci_{LD_BLOCK}_maf.txt"
                            )
                            maf_BB = np.loadtxt(
                                "/scratch/negishi/chen4422/hapnest/Simulation_update/"
                                f"simu_region_1mb/risk_loci_ld_afr/maf_loci_{LD_BLOCK}_maf.txt"
                            )
                        except Exception as e:
                            print(f"[MAF] read error LD_BLOCK_{LD_BLOCK}: {e}")
                            continue

                        # Align lengths and subset MAFs
                        maf_EU_subset = maf_EU[keep_idx]
                        maf_BB_subset = maf_BB[keep_idx]

                        if maf_EU_subset.shape[0] != m or maf_BB_subset.shape[0] != m:
                            print(
                                f"[MAF] length mismatch (expected {m}): "
                                f"EU={maf_EU_subset.shape[0]}, BB={maf_BB_subset.shape[0]}"
                            )
                            continue
                        ...
                        print(
                            f"[DONE] num_causal={num_causal} | "
                            f"causal_index={causal_index} ({causal_index_name}) | "
                            f"External_index={External_index} ({External_index_name or 'None'}) | "
                            f"LD_BLOCK={LD_BLOCK} | h2_num={h2_num} | "
                            f"minutes={time_taken:.2f}",
                            flush=True
                        )
