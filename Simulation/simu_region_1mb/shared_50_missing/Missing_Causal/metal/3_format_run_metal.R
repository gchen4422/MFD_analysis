    ...
    for (num in num_causal_vec) {
      for (h2 in h2_num_vec) {
        ...
        wrk_dir <- paste0(
          ...
          "shared_50_missing/Missing_Causal/",
        ...
        for (LD_BLOCK in LD_BLOCK_range) {
          ...
          zfile<-read.table(paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_all/causal_num_",num,"/summary_data/CAUSAL_",num,"_LOCI_",LD_BLOCK,"_h2_",h2),header=T)
          ...
          zfile_susiex_input = fread(paste0(data_dir,"CAUSAL_",num,"_LOCI_",LD_BLOCK,"_h2_",h2,"_eur"))
          zfile$EU_missing = ifelse(zfile$RSID %in%zfile_susiex_input$RSID,0,1)
          zfile$BB_missing = zfile$EU_missing
          
          
          missing_index_3 = which(zfile$EU_missing!=0|zfile$BB_missing!=0)
...
base_dir <- "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50_missing/Missing_Causal"
