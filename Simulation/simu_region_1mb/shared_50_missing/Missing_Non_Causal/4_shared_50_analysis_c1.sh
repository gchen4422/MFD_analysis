...
if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
	Rscript --verbose /scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50_missing/Missing_Non_Causal/4_shared_50_analysis.R ${gene} ${h2} 1 ${causal_index} ${External_index}
fi
done
done
done
