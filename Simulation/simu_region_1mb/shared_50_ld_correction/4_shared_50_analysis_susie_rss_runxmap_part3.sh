...
if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
	Rscript --verbose /scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50_ld_correction/4_shared_50_analysis_susie_rss_runxmap.R ${gene} ${h2} 3 ${flip_prop} ${rho_num}
fi
done
done
done
