...
for (num_causal in num_causal_range) {
  for (rho_num in rho_num_range) {
    for (flip_prop in flip_prop_range) {
      for (LD_BLOCK in LD_BLOCK_range) {
        for (h2_num in h2_num_range) {
            ...
            output_dir<-paste0(data_dir_flipped,"slalom/")
            system(paste0("mkdir -p ",output_dir))
            
            
            
            
            summary_stat_1_susiex = fread(paste0(data_dir_flipped,"CAUSAL_",num_causal,"_LOCI_",LD_BLOCK,"_h2_",h2_num,"_eur"))
            summary_stat_2_susiex = fread(paste0(data_dir_flipped,"CAUSAL_",num_causal,"_LOCI_",LD_BLOCK,"_h2_",h2_num,"_afr"))
