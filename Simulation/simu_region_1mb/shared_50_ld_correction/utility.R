...
Set_Power_fun<-function(power_summary){
  ...
  p_power_bar = p_power_bar + theme_bw()+custom_theme()+  theme(
...
ROC_ancestry_fun<-function(all_ROC_data_dataframe){
  p = ggplot(all_ROC_data_dataframe,aes(x = Power, y = 1 - FDR )) + geom_line(aes(x = Power, y = 1 - FDR,linetype = Ancestry,color = Method),size=0.8)+scale_color_manual(values=c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5","SuSiE_weighted"="#B2D3A4","SuSiE_merged"="#9FBA95","Paintor"="#fc8d62","MultiSuSiE"="#e78ac3","SuSiEx"="#E89DA0","XMAP"="#ffd92f","CARMAX"="#f2b56e"))+ scale_linetype_manual(values=c("White British" = "dashed", "Black British"="solid")) +
    xlab("Recall") + 
    ylab("Precision") + 
    theme_bw() + 
    theme( legend.position = "bottom") + 
...
FDR_Power_ancestry_fun<-function(FDR_Power){
  p =  ggplot(FDR_Power, aes(FDR, Power,fill = Method,pattern = Ancestry)) + 
...
PIP_calibration_shared_byh2_fun<-function(PIP_calibration){
  p = ggplot(data = PIP_calibration,aes(x = mean_pip,y = obs_frq)) + theme_bw()+ geom_pointrange(aes(ymin = obs_min,ymax = obs_max),col="grey") +geom_point(col="black")+geom_abline(intercept = 0, slope = 1,col="red")+ylab("Observed Frequency")+xlab("Mean PIP")+facet_wrap(vars(Method),labeller=label_parsed,nrow = 1)			 
  p = p + scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))+scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))
  p = p +	custom_theme() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2)
  return(p)
...
PIP_calibration_ancestry_fun<-function(PIP_calibration){
  p = ggplot(data = PIP_calibration,aes(x = mean_pip,y = obs_frq)) + theme_bw()+ geom_pointrange(aes(ymin = obs_min,ymax = obs_max),col="grey") +geom_point(col="black")+geom_abline(intercept = 0, slope = 1,col="red")+ylab("Observed Frequency")+xlab("Mean PIP")+facet_grid(vars(Method),labeller = label_parsed)			 
  p = p + scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))+scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))
  p = p +	custom_theme() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2)
  return(p)
...
PIP_calibration_ancestry_byh2_fun<-function(PIP_calibration){
  p = ggplot(data = PIP_calibration,aes(x = mean_pip,y = obs_frq)) + theme_bw()+ geom_pointrange(aes(ymin = obs_min,ymax = obs_max),col="grey") +geom_point(col="black")+geom_abline(intercept = 0, slope = 1,col="red")+ylab("Observed Frequency")+xlab("Mean PIP")+facet_wrap(~Method, labeller = label_parsed, nrow = 2)		 
  p = p + scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))+scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))
  p = p +	custom_theme() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2)
  return(p)
