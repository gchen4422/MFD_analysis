...
Set_Size_fun<-function(all_Set_data_dataframe,upper_limit){
  p_size_box = ggplot(data=all_Set_data_dataframe, aes(x = reorder(Method,Size,median), y =Size, fill=Method)) +
    ...
    scale_fill_manual(values=c("SuSiE_weighted"="#1b9e77","SuSiE_merged"="#66c2a5","Het_weighted"="#377eb8","Het_merged"="#9ecae1","SuSiE_rss_weighted"="#e7298a","SuSiE_rss_merged"="#f781bf","Slalom_weighted"="#d95f02","Slalom_merged"="#fdb863"), guide=FALSE) +
    ylab("Set Size") +
    xlab("") +
...
Set_Size_fun_order <- function(all_Set_data_dataframe, upper_limit) {
  
  # 1. Define the plotting order
  method_order <- c("SuSiE_weighted", "SuSiE_merged", 
                    "Het_weighted", "Het_merged", 
                    "SuSiE_rss_weighted", "SuSiE_rss_merged", 
                    "Slalom_weighted", "Slalom_merged")
  
  # 2. Update the dataframe to use this factor order
  all_Set_data_dataframe$Method <- factor(all_Set_data_dataframe$Method, levels = method_order)
  
  p_size_box = ggplot(data = all_Set_data_dataframe, 
                      aes(x = Method, y = Size, fill = Method)) + # Preserve method order
    
    geom_boxplot(aes(fill = Method), outlier.size = 0.1) +
    coord_cartesian(ylim = c(0, upper_limit)) +
    
    # 3. Apply colors (Order matches the factor levels)
    scale_fill_manual(
      values = c("SuSiE_weighted"="#1b9e77", "SuSiE_merged"="#66c2a5",
                 "Het_weighted"="#377eb8", "Het_merged"="#9ecae1",
                 "SuSiE_rss_weighted"="#e7298a", "SuSiE_rss_merged"="#f781bf",
                 "Slalom_weighted"="#d95f02", "Slalom_merged"="#fdb863"), 
      guide = "none" # Use "none" instead of FALSE in newer ggplot versions
    ) +
...
Set_Power_fun<-function(power_summary){
  p_power_bar = ggplot(data=power_summary,aes(x = Method, y =Power_name,fill=Method))+geom_bar(stat = "identity", position = "dodge", aes(fill=Method),alpha = 1.2)+scale_fill_manual(values=c("SuSiE_weighted"="#1b9e77","SuSiE_merged"="#66c2a5","Het_weighted"="#377eb8","Het_merged"="#9ecae1","SuSiE_rss_weighted"="#e7298a","SuSiE_rss_merged"="#f781bf","Slalom_weighted"="#d95f02","Slalom_merged"="#fdb863"),guide=FALSE)+
...
Set_Power_fun <- function(power_summary){
  ...
  # 1. Define specific order (Optional, ensures Plot matches Legend order)
  method_levels <- c("SuSiE_weighted", "SuSiE_merged", "Het_weighted", "Het_merged", 
                     "SuSiE_rss_weighted", "SuSiE_rss_merged", "Slalom_weighted", "Slalom_merged")
  
  # Ensure the factor levels are set correctly
  power_summary$Method <- factor(power_summary$Method, levels = method_levels)
  
  p_power_bar = ggplot(data = power_summary, 
                       aes(x = Method, y = Power_name, fill = Method)) +
    
    geom_bar(stat = "identity", position = "dodge", alpha = 1.2) +
    
    # 2. Display the legend
    scale_fill_manual(
      values = c("SuSiE_weighted"="#1b9e77", "SuSiE_merged"="#66c2a5",
                 "Het_weighted"="#377eb8", "Het_merged"="#9ecae1",
                 "SuSiE_rss_weighted"="#e7298a", "SuSiE_rss_merged"="#f781bf",
    ...
    # 3. Format Legend (Force 2 rows for compactness)
  ...
  p_power_bar = p_power_bar + 
    geom_text(aes(label = round(Power_name, 2)), 
              position = position_dodge(width = 1), 
              vjust = -0.5, 
              size = 5 * 5 / 14)
  
  # Apply Theme
  p_power_bar = p_power_bar + custom_theme() + 
    theme(
      axis.text.x = element_text(size = 3, face = "bold"),
      legend.position = "bottom" # Ensure legend is visible at the bottom
    ) + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2)
...
ROC_shared_fun <- function(all_ROC_data_dataframe) {
  p <- ggplot(all_ROC_data_dataframe, aes(x = Power, y = 1 - FDR)) + 
    ...
    scale_color_manual(values = c("SuSiE_weighted"="#1b9e77","SuSiE_merged"="#66c2a5","Het_weighted"="#377eb8","Het_merged"="#9ecae1","SuSiE_rss_weighted"="#e7298a","SuSiE_rss_merged"="#f781bf","Slalom_weighted"="#d95f02","Slalom_merged"="#fdb863")) + 
    xlab("Recall") + 
    ylab("Precision") + 
    theme_bw() + 
...
ROC_ancestry_fun<-function(all_ROC_data_dataframe){
  p = ggplot(all_ROC_data_dataframe,aes(x = Power, y = 1 - FDR )) + geom_line(aes(x = Power, y = 1 - FDR,linetype = Ancestry,color = Method),size=0.8)+scale_color_manual(values=c("SuSiE_weighted"="#1b9e77","SuSiE_merged"="#66c2a5","Het_weighted"="#377eb8","Het_merged"="#9ecae1","SuSiE_rss_weighted"="#e7298a","SuSiE_rss_merged"="#f781bf","Slalom_weighted"="#d95f02","Slalom_merged"="#fdb863"))+ scale_linetype_manual(values=c("White British" = "dashed", "Black British"="solid")) +
    xlab("Recall") + 
    ylab("Precision") + 
    theme_bw() + 
    theme( legend.position = "bottom") + 
...
FDR_Power_ancestry_fun<-function(FDR_Power){
  p =  ggplot(FDR_Power_ancestry, aes(FDR, Power,fill = Method,pattern = Ancestry)) + 
    ...
    scale_fill_manual(name = "Method",values = c("SuSiE_weighted"="#1b9e77","SuSiE_merged"="#66c2a5","Het_weighted"="#377eb8","Het_merged"="#9ecae1","SuSiE_rss_weighted"="#e7298a","SuSiE_rss_merged"="#f781bf","Slalom_weighted"="#d95f02","Slalom_merged"="#fdb863"),guide=FALSE)+   
...
FDR_Power<-function(dat){
  res<-do.call(rbind,lapply(c(0.01,0.05,0.1,0.5),function(x){
    intermediate_data<-dat %>% dplyr::group_by( Method,h2,causal_num) %>% filter(abs(FDR - x) == min(abs(FDR - x))) %>%  dplyr::slice(which.max(Power))
...
create_obs_frq<-function(pip,signal_num,col_names){
  ...
  PIP_calibration<-pip_signal_long%>% group_by(Method,causal_num,gr=cut(PIP, breaks= seq(0, 1, by = 0.1)))%>%dplyr::summarize(mean_pip = mean(PIP),obs_frq = mean(Signal),N_obs= n())
