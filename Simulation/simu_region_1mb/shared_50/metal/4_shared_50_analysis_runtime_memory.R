...
susie_meta_df_perf <- data.frame(
  NUM_SNP = rep(nrow(zfile), 2),
  Method  = c("MESuSiE_Weighted", "MESuSiE_Merged"),
  Time_Min = c(mem_w$Elapsed_Time_sec/60, mem_m$Elapsed_Time_sec/60),
  Peak_RAM_MB = c(mem_w$Peak_RAM_Used_MiB, mem_m$Peak_RAM_Used_MiB)
)


time_out_name<-paste0(result_dir,"Runtime_memory_CAUSAL_",num_causal,"_LOCI_",LD_BLOCK,"_h2_",h2_num,"_metal.txt")
write.table(susie_meta_df_perf,time_out_name,col.names = F,row.names = F,quote=F)
