...
res_out<-"shared_50_baseline_updated.RData"
library(ggplot2)
library(ggrepel)
library(grid)
library(egg)
...
for(causal_num in 1:3){
  for(h2 in 1:2){
    ...
    MESuSiE_ROC <- SuSiE_ROC <- Paintor_ROC <- susiex_ROC <- XMAP_ROC <- multisusie_ROC <- 
      carmax_ROC <- pipsort_ROC <-
      MESuSiE_ROC_Either <- SuSiE_ROC_Either <- Paintor_ROC_Either <- susiex_ROC_Either <- 
      XMAP_ROC_Either <- multisusie_ROC_Either <- carmax_ROC_Either <- pipsort_ROC_Either <-
      MESuSiE_ROC_EU <- MESuSiE_ROC_BB <- SuSiE_ROC_EU <- SuSiE_ROC_BB <- Paintor_ROC_EU <- 
      Paintor_ROC_BB <- susiex_ROC_EU <- susiex_ROC_BB <- XMAP_ROC_EU <- XMAP_ROC_BB <- 
      multisusie_ROC_EU <- multisusie_ROC_BB <- carmax_ROC_EU <- carmax_ROC_BB <- 
      pipsort_ROC_EU <- pipsort_ROC_BB <- 
      empty_mat
    ...
    for(LOCI_num in replicate_set){
      ...
      susiex_res_name<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50/susiex_result/","SuSiEx_CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95.snp")
      susiex_cred_name<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50/susiex_result/","SuSiEx_CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95.cs")
      ...
      ld_dir = paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50/causal_num_",causal_num,"/")
      ...
      multisusie_dir = "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50/multi_susie_result/"
      ...
      #format pipsort results
      
      pipsort_outname = paste0(result_dir,paste0("CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2),"_pipsort")
      pipsort_cs_set1 = fread(paste0(pipsort_outname,"_study0_set.txt"),header = F)$V1
      pipsort_cs_set2 = fread(paste0(pipsort_outname,"_study1_set.txt"),header = F)$V1
      pipsort_cs = match(union(pipsort_cs_set1,pipsort_cs_set2),paintor_res$SNP)
      pipsort_pip_either = fread(paste0(pipsort_outname,"_global_pips.txt"))$global_pips
      #pipsort_pip_notshared = fread(paste0(pipsort_outname,"_not_shared_pips.txt"))$not_shared_pip
      pipsort_pip_eur = fread(paste0(pipsort_outname,"_study0_post.txt"))$Prob_in_pCausalSet
      pipsort_pip_afr = fread(paste0(pipsort_outname,"_study1_post.txt"))$Prob_in_pCausalSet
      pipsort_pip_shared = fread(paste0(pipsort_outname,"_shared_pips.txt"))$shared_pip
      ...
      pipsort_SET = compute_set_susie(unique(pipsort_cs),ifelse(paintor_res$Signal!=0,1,0))	
      
      
      
      
      set_res_all<-rbind(set_res_all,rbind(c(MESuSiE_SET,"MESuSiE"),c( SuSiE_SET,"SuSiE"),c(Paintor_SET$Paintor_set,"Paintor"),c(susiex_SET,"SuSiEx"),c(XMAP_SET,"XMAP"),c(multisusie_SET,"MultiSuSiE"),c(carmax_SET,"CARMAX"),c(pipsort_SET,"PIPSORT")))
      ...
      pipsort_ROC = pipsort_ROC + compute_ROC(data.frame("SNP" =paintor_res$SNP ,"PIP" = pipsort_pip_shared),ifelse(paintor_res$Signal==3,1,0))
      ...
      pipsort_ROC_Either = pipsort_ROC_Either + compute_ROC(data.frame("SNP" =paintor_res$SNP ,"PIP" = pipsort_pip_either),ifelse(paintor_res$Signal==0,1,0))
      ...
      pipsort_ROC_EU = pipsort_ROC_EU + compute_ROC(data.frame("SNP" =paintor_res$SNP ,"PIP" = pipsort_pip_eur),ifelse(paintor_res$Signal==1,1,0))
      pipsort_ROC_BB = pipsort_ROC_BB + compute_ROC(data.frame("SNP" =paintor_res$SNP ,"PIP" = pipsort_pip_afr),ifelse(paintor_res$Signal==2,1,0))
      ...
      ##Sumstat
      set_mat<-data.frame(matrix(0,ncol = 8,nrow = nrow(paintor_res)))
      colnames(set_mat)<-c("MESuSiE_cs","SuSiE_cs","Paintor_cs","SuSiEx_cs","XMAP_cs","MultiSuSiE_cs","CARMAX_cs","PIPSORT_cs")
      ...
      set_mat$PIPSORT_cs[pipsort_cs]<-1
      
      
      
      locus_sumstat<-paintor_res%>%mutate(Paintor_cs = set_mat$Paintor_cs,CARMAX_cs = set_mat$CARMAX_cs,CARMAX_Either = carmax_pip_either, CARMAX_Shared = carmax_pip_shared, CARMAX_WB = carmax_pip_eur, CARMAX_BB = carmax_pip_afr,PIPSORT_cs = set_mat$PIPSORT_cs,PIPSORT_Either = pipsort_pip_either, PIPSORT_Shared = pipsort_pip_shared, PIPSORT_WB = pipsort_pip_eur, PIPSORT_BB = pipsort_pip_afr ,
    ...
    ####################################### 
    # Either signal part
    roc_data_list <- list(MESuSiE_ROC_Either = MESuSiE_ROC_Either, SuSiE_ROC_Either = SuSiE_ROC_Either, Paintor_ROC_Either = Paintor_ROC_Either,susiex_ROC_Either = susiex_ROC_Either,XMAP_ROC_Either = XMAP_ROC_Either, multisusie_ROC_Either = multisusie_ROC_Either, carmax_ROC_Either= carmax_ROC_Either, pipsort_ROC_Either = pipsort_ROC_Either)
    method_names <- c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")
    ...
    # Shared signal part
    roc_data_list <- list(MESuSiE_ROC = MESuSiE_ROC, SuSiE_ROC = SuSiE_ROC, Paintor_ROC = Paintor_ROC,susiex_ROC = susiex_ROC,XMAP_ROC = XMAP_ROC, multisusie_ROC = multisusie_ROC,carmax_ROC= carmax_ROC, pipsort_ROC = pipsort_ROC)
    method_names <- c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")
...
all_Set_data_dataframe<-all_Set_data_dataframe%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")))
...
set_power_summary<-set_power_summary%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")))
...
PIP_calibration_either<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_Either,SuSiE_Either, Paintor_PIP, SuSiEx_PIP, XMAP_PIP,MultiSuSiE_PIP,CARMAX_Either,PIPSORT_Either),c(1,2,3),c("MESuSiE_Either","SuSiE_Either","Paintor_PIP","SuSiEx_PIP","XMAP_PIP","MultiSuSiE_PIP","CARMAX_Either","PIPSORT_Either"))
PIP_calibration_either<-PIP_calibration_either%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Either","SuSiE" = "SuSiE_Either","Paintor" = "Paintor_PIP", "SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP","CARMAX" = "CARMAX_Either","PIPSORT" = "PIPSORT_Either"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor", "SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT"))


####PIP Shared	
PIP_calibration_Shared<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_Shared,SuSiE_Shared,Paintor_PIP, SuSiEx_PIP, XMAP_PIP, MultiSuSiE_PIP,CARMAX_Shared,PIPSORT_Shared),c(3),c("MESuSiE_Shared","SuSiE_Shared","Paintor_PIP","SuSiEx_PIP","XMAP_PIP","MultiSuSiE_PIP","CARMAX_Shared","PIPSORT_Shared"))
PIP_calibration_Shared<-PIP_calibration_Shared%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Shared","SuSiE" = "SuSiE_Shared","Paintor" = "Paintor_PIP","SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP","CARMAX" = "CARMAX_Shared","PIPSORT" = "PIPSORT_Shared"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT"))
...
PIP_calibration_WB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_WB,Paintor_WB,SuSiEx_WB, XMAP_WB, MultiSuSiE_WB, CARMAX_WB, PIPSORT_WB),c(1),c("MESuSiE_WB","Paintor_WB","SuSiEx_WB","XMAP_WB", "MultiSuSiE_WB","CARMAX_WB","PIPSORT_WB"))
PIP_calibration_BB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_BB,Paintor_BB,SuSiEx_BB, XMAP_BB, MultiSuSiE_BB, CARMAX_BB, PIPSORT_BB),c(2),c("MESuSiE_BB","Paintor_BB","SuSiEx_BB","XMAP_BB", "MultiSuSiE_BB","CARMAX_BB","PIPSORT_BB"))

PIP_calibration_ancestry<-rbind(PIP_calibration_WB,PIP_calibration_BB)
PIP_calibration_ancestry<-PIP_calibration_ancestry%>%mutate(Method = fct_recode(Method, "MESuSiE_WB" = "MESuSiE_WB","MESuSiE_BB" = "MESuSiE_BB","Paintor_WB" = "Paintor_WB","Paintor_BB" = "Paintor_BB","SuSiEx_WB" = "SuSiEx_WB","SuSiEx_BB" = "SuSiEx_BB","XMAP_WB" = "XMAP_WB","XMAP_BB" = "XMAP_BB", "MultiSuSiE_WB" = "MultiSuSiE_WB", "MultiSuSiE_BB" = "MultiSuSiE_BB","CARMAX_WB" = "CARMAX_WB","CARMAX_BB" = "CARMAX_BB","PIPSORT_WB" = "PIPSORT_WB", "PIPSORT_BB" = "PIPSORT_BB" ))%>%mutate(Method = fct_relevel(Method,"MESuSiE_WB","MESuSiE_BB","Paintor_WB", "Paintor_BB","SuSiEx_WB","SuSiEx_BB","XMAP_WB","XMAP_BB","MultiSuSiE_WB", "MultiSuSiE_BB","CARMAX_WB","CARMAX_BB","PIPSORT_WB", "PIPSORT_BB"))
levels(PIP_calibration_ancestry$Method)<-c(paste0("MESuSiE~","WB"),paste0("MESuSiE~","BB"),paste0("Paintor~","WB"),paste0("Paintor~","BB"),paste0("SuSiEx~","WB"),paste0("SuSiEx~","BB"),paste0("XMAP~","WB"),paste0("XMAP~","BB"),paste0("MultiSuSiE~","WB"),paste0("MultiSuSiE~","BB"),paste0("CARMAX~","WB"),paste0("CARMAX~","BB"),paste0("PIPSORT~","WB"),paste0("PIPSORT~","BB"))
#######################################
#
#   ROC data 
#
...
ancestry_all_ROC_data_dataframe<-ancestry_all_ROC_data_dataframe%>%mutate(Method = fct_recode(Method,
                                                                                              ...
                                                                                              "CARMAX WB" = "CARMAX_EU",
                                                                                              "CARMAX BB" = "CARMAX_BB",
                                                                                              "PIPSORT WB" = "PIPSORT_EU",
                                                                                              "PIPSORT BB" = "PIPSORT_BB",
                                                                                              ))
...
FDR_Power_shared<-FDR_Power(shared_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")))
FDR_Power_shared%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))


FDR_Power_either<-FDR_Power(either_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX","PIPSORT")))
FDR_Power_either%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))

FDR_Power_ancestry<-FDR_Power(ancestry_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method,"MESuSiE BB","MESuSiE WB", "SuSiE BB" ,"SuSiE WB","Paintor BB" ,"Paintor WB","SuSiEx BB" ,"SuSiEx WB","XMAP WB","XMAP BB","MultiSuSiE WB","MultiSuSiE BB","CARMAX WB", "CARMAX BB","PIPSORT WB","PIPSORT BB"))
...
colnames(time_all)<-c("MESuSiE","SuSiE","Paintor","XMAP","SuSiEx","CARMAX","PIPSORT","MultiSuSiE","NUM_SNP")
