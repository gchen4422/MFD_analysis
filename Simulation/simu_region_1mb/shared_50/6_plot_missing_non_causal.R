...
Compare_Plot_Missing <- function(res_dir, plot_dir_name, res_out_1, res_out_2,pattern_name_1, pattern_name_2,pattern_name_3) {
  ...
  p_ROC_either <- ROC_ancestry_Compare_fun(ROC_either%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","SuSiE_merged","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")), pattern_input_line)
  ...
  p_ROC_shared<-ROC_ancestry_Compare_fun(ROC_shared%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","SuSiE_merged","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_line )
  ...
  p_FDR_Power_WB<-FDR_Power_Compare_fun_supp(FDR_Power_ancestry[grep("WB",FDR_Power_ancestry$Method),] %>%
                                          ...
                                          mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","SuSiE_merged","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_Power)
  ...
  p_ROC_WB<-ROC_ancestry_Compare_fun(ROC_ancestry[grep("WB",ROC_ancestry$Method),]%>%
                                       ...
                                       mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","SuSiE_merged","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_line )
...
res_dir<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/res_summary/")
plot_dir_name<-"Missing_Non_Causal_meta_updated"
...
res_dir<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/res_summary/")
plot_dir_name<-"Missing_Non_Causal_Ref_meta_updated"
...
for (External_index in 1:2) {
  ...
  plot_dir <- paste0(
    res_dir,
    "Figure/Missing_Causal", External_index_name, "_updated_meta/"
  )
  system(paste0("mkdir -p ", plot_dir))
  ...
  for (causal_index in 1:2) {
    ...
    ggsave(
      ...
      plot = p_out,
      dpi = 500, units = "mm", height = 180, width = 210
    )
  }
}
