...
multisusie_dir <- "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_50/multi_susie_result/runtime/"

# Initialize list for speed
results_list <- list()
...
for(causal_num in 1:3){
  for(h2 in 1:2){
    ...
    if(!dir.exists(result_dir)) next
    
    # Get completed replicates based on the main result files
    all_files <- list.files(result_dir)
    target_files <- paste0("MESuSiE_CAUSAL_", causal_num, "_LOCI_", 1:100, "_h2_", h2, ".RData")
    # Identify which Locus numbers (1-100) actually exist
    replicate_set <- which(target_files %in% all_files)
    
    for(LOCI_num in replicate_set){
      
      # --- 1. Define File Paths ---
      # Main file (MESuSiE, SuSiE, Paintor, XMAP, SuSiEx, CARMAX)
      f_main <- paste0(result_dir, "Runtime_memory_CAUSAL_", causal_num, "_LOCI_", LOCI_num, "_h2_", h2, ".txt")
      # MultiSuSiE file
      f_multi <- paste0(multisusie_dir, "MultiSuSiE_CAUSAL_", causal_num, "_LOCI_", LOCI_num, "_h2_", h2, "_output_runtime_memory.txt")
      # Metal file (Contains NSNP info)
      f_metal <- paste0(result_dir, "Runtime_memory_CAUSAL_", causal_num, "_LOCI_", LOCI_num, "_h2_", h2, "_metal.txt")
      
      df_parts <- list()
      nsnp_val <- NA # Default if metal file is missing
      
      # --- 2. Read Main Results ---
      if(file.exists(f_main)){
        df_parts[[1]] <- fread(f_main)
      }
      
      # --- 3. Read MultiSuSiE ---
      if(file.exists(f_multi)){
        df_parts[[2]] <- fread(f_multi)
      }
      
      # --- 4. Read Metal & Extract NSNP ---
      if(file.exists(f_metal)){
        # Metal file often has no header (V1=NSNP, V2=Method, V3=Time, V4=RAM)
        d_metal <- fread(f_metal, header = FALSE)
        
        if(nrow(d_metal) > 0){
          # Extract NSNP count to share with other methods
          nsnp_val <- mean(d_metal$V1, na.rm = TRUE)
          
          # Format Metal data to match others
          d_metal_clean <- d_metal %>%
            dplyr::rename(NSNP_Raw = V1, Method = V2, Time_Min = V3, Peak_RAM_Used_MiB = V4) %>%
            mutate(Elapsed_Time_sec = Time_Min * 60) %>%
            dplyr::select(Method, Time_Min, Elapsed_Time_sec, Peak_RAM_Used_MiB)
          
          df_parts[[3]] <- d_metal_clean
        }
      }
      
      # --- 5. Combine & Annotate ---
      if(length(df_parts) > 0){
        # Merge all parts for this locus
        locus_combined <- rbindlist(df_parts, fill = TRUE)
        ...
        results_list[[length(results_list) + 1]] <- locus_combined
      }
      
      # cat(LOCI_num, " ") # Optional progress print
    }
    cat(paste("Done Causal:", causal_num, "H2:", h2, "\n"))
  }
}

# Final bind
time_all_completet <- rbindlist(results_list)

# Check result
print(head(time_all_completet))


load(paste0(res_dir,"shared_50_baseline_updated_meta.RData"))
time_all_long_multisusie <- time_all %>%
  pivot_longer(cols = MESuSiE:MultiSuSiE, names_to = "Method", values_to = "Time")%>%mutate(Method = factor(Method, levels = c("MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE"))) %>% filter(Method == "MultiSuSiE") %>% arrange(NUM_SNP)
time_all_long_multisusie = time_all_long_multisusie %>% group_by(NUM_SNP) %>%
  summarise(
    Mean_Time = mean(Time, na.rm = TRUE),
    .groups = "drop"
  )

# 1. Rename the methods in the raw data
time_all_clean <- time_all_completet %>%
  mutate(Method = case_when(
    Method == "MESuSiE_Weighted" ~ "SuSiE_weighted",
    Method == "MESuSiE_Merged"   ~ "SuSiE_merged",
    TRUE                         ~ Method # Keep others the same
  ))

# 2. Summarize Data (Group by NSNP and Method)
summary_perf <- time_all_clean %>%
  dplyr::rename(NUM_SNP = NSNP) %>% 
...
summary_perf <- summary_perf %>%
  rows_update(
    time_all_long_multisusie %>% mutate(Method = "MultiSuSiE"),
    by = c("NUM_SNP", "Method")
  )
# 3. Define Colors with NEW Names
my_colors <- c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5","SuSiE_weighted"="#B2D3A4","SuSiE_merged"="#9FBA95","Paintor"="#fc8d62","MultiSuSiE"="#e78ac3","SuSiEx"="#E89DA0","XMAP"="#ffd92f","CARMAX"="#f2b56e")


summary_perf
summary_perf$Method <- factor(
  summary_perf$Method,
  levels = method_levels_base_2
)
# 4. Plot Runtime
...
# 5. Plot Peak Memory
p_mem <- ggplot(summary_perf, aes(x = NUM_SNP, y = Mean_RAM, color = Method)) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_smooth(method = "loess", se = FALSE, span = 0.8) +
  scale_y_log10() +
  scale_color_manual(values = my_colors) +
...
# 6. Combine
p_perf_out <- (p_time / p_mem / p_time_pipsort) + 
  plot_layout(guides = "collect") +
  plot_annotation(
    tag_levels = 'a'
  ) & 
  theme(
    legend.position = "bottom",
    plot.tag = element_text(face = "bold") # Optional: Makes the (a) bold
  )

print(p_perf_out)
ggsave(paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/res_summary/Figure/shared_50_baseline_updated_meta_updated_xmap/Time.pdf"),p_perf_out,height = 180, width = 180,units = "mm",dpi=600)
...
perf_method_summary$n_regions = 100
...
writeData(wb, sheet_name, "Supplementary Table: Runtime and Memory Summary Table", startRow = 1)
addStyle(wb, sheet_name, title_style, rows = 1, cols = 1)

# Write the Combined Data Table starting at Row 2
writeData(wb, sheet_name, perf_method_summary, startRow = 2)
...
FDR_Power_Compare_fun<-function(FDR_Power,pattern_input){
  p = ggplot(FDR_Power, aes(FDR, Power,fill = Method,pattern = Pattern)) + 
    ...
    scale_fill_manual(name = "Method",values = c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5","Paintor"="#fc8d62","MultiSuSiE"="#e78ac3","SuSiEx"="#E89DA0","XMAP"="#ffd92f","CARMAX"="#f2b56e"),guide=FALSE)+
...
ROC_ancestry_Compare_fun<-function(all_ROC_data_dataframe,pattern_input){
  p = ggplot(all_ROC_data_dataframe,aes(x = Power, y = 1 - FDR )) + geom_line(aes(x = Power, y = 1 - FDR,linetype = Pattern,color = Method),size=0.8)+scale_color_manual(values=c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5","Paintor"="#fc8d62","MultiSuSiE"="#e78ac3","SuSiEx"="#E89DA0","XMAP"="#ffd92f","CARMAX"="#f2b56e"))+ scale_linetype_manual(values=pattern_input) +
    xlab("Recall") + 
    ylab("Precision") + 
    theme_bw() + 
...
Compare_Plot_Missing <- function(res_dir, plot_dir_name, res_out_1, res_out_2,pattern_name_1, pattern_name_2,pattern_name_3) {
  ...
  # Function to generate ROC ancestry comparison plot (defined elsewhere)
  p_ROC_either <- ROC_ancestry_Compare_fun(ROC_either%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")), pattern_input_line)
  ...
  p_FDR_Power_shared<-FDR_Power_Compare_fun(FDR_Power_shared,pattern_input_Power)
  p_ROC_shared<-ROC_ancestry_Compare_fun(ROC_shared%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_line )
  ...
  p_FDR_Power_WB<-FDR_Power_Compare_fun(FDR_Power_ancestry[grep("WB",FDR_Power_ancestry$Method),] %>%
                                          mutate(Method = case_when(
                                            ...
                                            TRUE ~ Method
                                            ))%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_Power)
  p_ROC_WB<-ROC_ancestry_Compare_fun(ROC_ancestry[grep("WB",ROC_ancestry$Method),]%>%
                                       mutate(Method = case_when(
                                         str_detect(Method, "^MESuSiE(\\b|_)") ~ "MESuSiE",
                                         str_detect(Method, "^MultiSuSiE(\\b|_)") ~ "MultiSuSiE",
                                         ...
                                         str_detect(Method, "^CARMAX(\\b|_)") ~ "CARMAX",
                                         TRUE ~ Method
                                       ))%>%mutate(Method = as_factor(Method) %>%fct_relevel("MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE","CARMAX")),pattern_input_line )
...
for(External_index in 1:2){
  ...
  plot_dir<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/res_summary/Figure/Missing_Causal",External_index_name,"/")
  system(paste0("mkdir -p ", plot_dir))
