...
res_out<-"shared_100_baseline.RData"
library(ggplot2)
library(ggrepel)
library(grid)
library(egg)
...
for(causal_num in 1:3){
  for(h2 in 1:2){
    ...
    replicate_set = c(1:78,80:100)
    #replicate_set = rep(1:41)
    empty_mat<-matrix(0,ncol =3,nrow = length(seq(1,0.01,-0.01)))
    MESuSiE_ROC<-SuSiE_ROC<- Paintor_ROC<- susiex_ROC<- XMAP_ROC<-multisusie_ROC<-MESuSiE_ROC_Either<-SuSiE_ROC_Either<-Paintor_ROC_Either<- susiex_ROC_Either<-XMAP_ROC_Either<-multisusie_ROC_Either <-MESuSiE_ROC_EU<-MESuSiE_ROC_BB<-SuSiE_ROC_EU<-SuSiE_ROC_BB<-Paintor_ROC_EU<-Paintor_ROC_BB<-susiex_ROC_EU<- susiex_ROC_BB<-XMAP_ROC_EU<- XMAP_ROC_BB<-multisusie_ROC_EU<- multisusie_ROC_BB<-empty_mat
    set_res_all<-c()
    pip_res_all<-c()  
    ...
    for(LOCI_num in replicate_set){
      ...
      multisusie_dir = "/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/shared_all/multi_susie_result/"
      ...
      set_res_all<-rbind(set_res_all,rbind(c(MESuSiE_SET,"MESuSiE"),c( SuSiE_SET,"SuSiE"),c(Paintor_SET$Paintor_set,"Paintor"),c(susiex_SET,"SuSiEx"),c(XMAP_SET,"XMAP"),c(multisusie_SET,"MultiSuSiE")))
      ...
      colnames(set_mat)<-c("MESuSiE_cs","SuSiE_cs","Paintor_cs","SuSiEx_cs","XMAP_cs","MultiSuSiE_cs")
    ...
    ####################################### 
    # Either signal part
    roc_data_list <- list(MESuSiE_ROC_Either = MESuSiE_ROC_Either, SuSiE_ROC_Either = SuSiE_ROC_Either, Paintor_ROC_Either = Paintor_ROC_Either,susiex_ROC_Either = susiex_ROC_Either,XMAP_ROC_Either = XMAP_ROC_Either, multisusie_ROC_Either = multisusie_ROC_Either)
    method_names <- c("MESuSiE", "SuSiE", "Paintor","SuSiEx", "XMAP", "MultiSuSiE")
    ...
    # Shared signal part
    roc_data_list <- list(MESuSiE_ROC = MESuSiE_ROC, SuSiE_ROC = SuSiE_ROC, Paintor_ROC = Paintor_ROC,susiex_ROC = susiex_ROC,XMAP_ROC = XMAP_ROC, multisusie_ROC = multisusie_ROC)
    method_names <- c("MESuSiE", "SuSiE", "Paintor","SuSiEx", "XMAP", "MultiSuSiE")
    ...
    method_names <- c("MESuSiE_EU", "MESuSiE_BB","SuSiE_EU", "SuSiE_BB","Paintor_EU", "Paintor_BB","SuSiEx_EU","SuSiEx_BB","XMAP_EU","XMAP_BB","MultiSuSiE_EU","MultiSuSiE_BB")
...
all_Set_data_dataframe<-all_Set_data_dataframe%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE")))
...
set_power_summary<-set_power_summary%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "Paintor","SuSiEx","XMAP","MultiSuSiE")))
...
PIP_calibration_either<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_Either,SuSiE_Either, Paintor_PIP, SuSiEx_PIP, XMAP_PIP,MultiSuSiE_PIP),c(1,2,3),c("MESuSiE_Either","SuSiE_Either","Paintor_PIP","SuSiEx_PIP","XMAP_PIP","MultiSuSiE_PIP"))
PIP_calibration_either<-PIP_calibration_either%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Either","SuSiE" = "SuSiE_Either","Paintor" = "Paintor_PIP", "SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor", "SuSiEx","XMAP","MultiSuSiE"))


####PIP Shared	
PIP_calibration_Shared<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_Shared,SuSiE_Shared,Paintor_PIP, SuSiEx_PIP, XMAP_PIP, MultiSuSiE_PIP),c(3),c("MESuSiE_Shared","SuSiE_Shared","Paintor_PIP","SuSiEx_PIP","XMAP_PIP","MultiSuSiE_PIP"))
PIP_calibration_Shared<-PIP_calibration_Shared%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Shared","SuSiE" = "SuSiE_Shared","Paintor" = "Paintor_PIP","SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE"))
...
PIP_calibration_WB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_WB,Paintor_WB,SuSiEx_WB, XMAP_WB, MultiSuSiE_WB),c(1),c("MESuSiE_WB","Paintor_WB","SuSiEx_WB","XMAP_WB", "MultiSuSiE_WB"))
PIP_calibration_BB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_BB,Paintor_BB,SuSiEx_BB, XMAP_BB, MultiSuSiE_BB),c(2),c("MESuSiE_BB","Paintor_BB","SuSiEx_BB","XMAP_BB", "MultiSuSiE_BB"))

PIP_calibration_ancestry<-rbind(PIP_calibration_WB,PIP_calibration_BB)
PIP_calibration_ancestry<-PIP_calibration_ancestry%>%mutate(Method = fct_recode(Method, "MESuSiE_WB" = "MESuSiE_WB","MESuSiE_BB" = "MESuSiE_BB","Paintor_WB" = "Paintor_WB","Paintor_BB" = "Paintor_BB","SuSiEx_WB" = "SuSiEx_WB","SuSiEx_BB" = "SuSiEx_BB","XMAP_WB" = "XMAP_WB","XMAP_BB" = "XMAP_BB", "MultiSuSiE_WB" = "MultiSuSiE_WB", "MultiSuSiE_BB" = "MultiSuSiE_BB"))%>%mutate(Method = fct_relevel(Method,"MESuSiE_WB","MESuSiE_BB","Paintor_WB", "Paintor_BB","SuSiEx_WB","SuSiEx_BB","XMAP_WB","XMAP_BB","MultiSuSiE_WB", "MultiSuSiE_BB"))
levels(PIP_calibration_ancestry$Method)<-c(paste0("MESuSiE~","WB"),paste0("MESuSiE~","BB"),paste0("Paintor~","WB"),paste0("Paintor~","BB"),paste0("SuSiEx~","WB"),paste0("SuSiEx~","BB"),paste0("XMAP~","WB"),paste0("XMAP~","BB"),paste0("MultiSuSiE~","WB"),paste0("MultiSuSiE~","BB"))
#######################################
#
#   ROC data 
#
...
FDR_Power_shared<-FDR_Power(shared_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE")))
FDR_Power_shared%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))


FDR_Power_either<-FDR_Power(either_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE","Paintor","SuSiEx","XMAP","MultiSuSiE")))
FDR_Power_either%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))

FDR_Power_ancestry<-FDR_Power(ancestry_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method,"MESuSiE BB","MESuSiE WB", "SuSiE BB" ,"SuSiE WB","Paintor BB" ,"Paintor WB","SuSiEx BB" ,"SuSiEx WB","XMAP WB","XMAP BB","MultiSuSiE WB","MultiSuSiE BB"))
...
colnames(time_all)<-c("MESuSiE","SuSiE","Paintor","XMAP","SuSiEx","MultiSuSiE","NUM_SNP")
