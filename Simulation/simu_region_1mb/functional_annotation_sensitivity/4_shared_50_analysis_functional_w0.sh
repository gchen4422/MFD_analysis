...
for H2 in "${H2_VALUES[@]}"; do
  for CAUSAL in "${CAUSAL_VALUES[@]}"; do
    ...
    Rscript --verbose /scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/functional_annotation_sensitivity/4_shared_50_analysis_functional_w0.R "${GENE}" "${H2}" "${CAUSAL}"
  done
done
