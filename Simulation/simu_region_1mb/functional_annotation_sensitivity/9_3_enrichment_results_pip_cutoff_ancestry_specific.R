###############################################################################
#
#  9_3_enrichment_results_pip_cutoff_ancestry_specific.R
#  (MFD revision / baseline_functional_annotation)
#
#  Adapted from shared_50_mesusie_inf/9_3_enrichmen_results_pip_cutoff_ancestry_specific.R
#
#  ANCESTRY-SPECIFIC (unique) signal enrichment. For methods with an ancestry
#  decomposition (MESuSiE, SuSiE, CARMAX) the "unique" PIP is the max of the two
#  ancestry-specific PIPs: pmax(WB, BB). Methods without a decomposition use
#  their single overall PIP. top_N_signal is set by MESuSiE unique PIP > thresh.
...
# unique/ancestry-specific PIP column per method
pip_cols <- c(MESuSiE = "MESuSiE_Unique", SuSiE = "SuSiE_Unique",
              ...
              CARMAX = "CARMAX_Unique")
method_levels <- names(pip_cols)
ref_pip <- "MESuSiE_Unique"   # defines top_N_signal
...
for (num_causal in 1:3) {
  for (h2_num in 1:2) {
    ...
    # build unique (ancestry-specific) PIPs = max over the two ancestries
    res_all <- res_all %>%
      mutate(MESuSiE_Unique = pmax(MESuSiE_WB, MESuSiE_BB),
             SuSiE_Unique   = pmax(SuSiE_WB,   SuSiE_BB),
             CARMAX_Unique  = pmax(CARMAX_WB,  CARMAX_BB))
    ...
    p <- ggplot(enrichment_results, aes(x = threshold, y = enrichment, color = method)) +
      ...
      labs(title = paste("Ancestry-specific enrichment | causal =", num_causal, "h2 =", h2_num),
           x = "MESuSiE unique-PIP threshold", y = "Enrichment (Signal / Background)") +
      theme_bw()
    ggsave(paste0(plot_dir, "Causal_num_", num_causal, "_h2_", h2_num,
                  "_Enrichment_pip_cutoff_unique.pdf"),
...
avg_plot <- ggplot(avg_enrichment, aes(x = threshold, y = avg_enrichment, color = method)) +
  ...
  facet_wrap(~ annotation, scales = "free_y") +
  scale_color_manual(values = method_colors) +
  labs(title = "Average ancestry-specific enrichment across all causal_num / h2",
       x = "MESuSiE unique-PIP threshold", y = "Average Enrichment (Signal / Background)") +
  theme_bw()
ggsave(paste0(plot_dir, "Summary_Enrichment_pip_cutoff_unique.pdf"),
       avg_plot, dpi = 500, width = 200, height = 130, units = 'mm')

save(combined_enrichment, avg_enrichment,
     file = paste0(plot_dir, "enrichment_pip_cutoff_unique.RData"))
cat("Done (ancestry-specific). Output in ", plot_dir, "\n")
