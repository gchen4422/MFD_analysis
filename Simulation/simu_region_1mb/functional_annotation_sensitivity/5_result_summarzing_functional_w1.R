## Usage: Rscript 5_result_summarzing_functional_w1.R w0
## Accepts w0, w1, or w2 as command-line argument (defaults to w0)
args <- commandArgs(TRUE)
w_val <- ifelse(length(args) >= 1, args[1], "w0")
cat("Running for weight:", w_val, "\n")
sub_dir <- paste0(w_val, "_startover")
...
res_out<-paste0("functional_annotation_sensitivity_", w_val, ".RData")
library(ggplot2)
library(ggrepel)
library(grid)
library(egg)
...
for(causal_num in 1:3){
  for(h2 in 1:2){
    ...
    data_dir<-paste0(wrk_dir,"summary_data/",sub_dir,"/")
    result_dir<-paste0(wrk_dir,"result/",sub_dir,"/")
    ...
    set3 = which(paste0("CAUSAL_",causal_num,"_LOCI_",1:100,"_h2_",h2,".fun.mcmc.paintor")%in%list.files())
    set4 = which(paste0("CAUSAL_",causal_num,"_LOCI_",1:100,"_h2_",h2,".all.fun.mcmc.paintor")%in%list.files())
    ...
    MESuSiE_ROC <- SuSiE_ROC <- SuSiE_weighted_ROC <- SuSiE_merged_ROC <- Paintor_ROC <-Paintor_fun_ROC <- Paintor_all_fun_ROC <-susiex_ROC <- XMAP_ROC <- multisusie_ROC <- carmax_ROC <-
      MESuSiE_ROC_Either <- SuSiE_ROC_Either <- SuSiE_weighted_ROC_Either <- SuSiE_merged_ROC_Either<- Paintor_ROC_Either <- Paintor_fun_ROC_Either  <- Paintor_all_fun_ROC_Either  <-susiex_ROC_Either <-
      XMAP_ROC_Either <- multisusie_ROC_Either <- carmax_ROC_Either <-
      MESuSiE_ROC_EU <- MESuSiE_ROC_BB <- SuSiE_ROC_EU <- SuSiE_ROC_BB <- Paintor_ROC_EU <- Paintor_ROC_BB <- Paintor_fun_ROC_EU <- Paintor_fun_ROC_BB <- Paintor_all_fun_ROC_EU <- Paintor_all_fun_ROC_BB <-
    ...
    for(LOCI_num in replicate_set){
      ...
      paintor_fun_res<-fread(paste0(result_dir,"CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,".fun.mcmc.paintor"))%>%dplyr::rename(SNP = RSID)
      paintor_all_fun_res<-fread(paste0(result_dir,"CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,".all.fun.mcmc.paintor"))%>%dplyr::rename(SNP = RSID)
      ...
      susiex_res_name<-paste0(result_dir,"SuSiEx_CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95.snp")
      susiex_cred_name<-paste0(result_dir,"SuSiEx_CAUSAL_",causal_num,"_LOCI_",LOCI_num,"_h2_",h2,"_output_cs95.cs")
      susiex_res = read.table(susiex_res_name, header = T)
      susiex_cs = read.table(susiex_cred_name,header = T)
      susiex_cs_num = if(nrow(susiex_cs) > 0) max(susiex_cs$CS_ID) else 0
      ...
      ld_dir = paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/functional_annotation_sensitivity/causal_num_",causal_num,"/")
      paintor_dir<-paste0(wrk_dir,"summary_data/",sub_dir,"/")
      ...
      multisusie_dir = paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/functional_annotation_sensitivity/multi_susie_result/",sub_dir,"/")
      ...
      Paintor_fun_SET = compute_set_pip(data.frame("SNP" =paintor_fun_res$SNP ,"PIP" = paintor_fun_res$Posterior_Prob,"CAUSAL"=ifelse(paintor_fun_res$Signal!=0,1,0)))
      Paintor_all_fun_SET = compute_set_pip(data.frame("SNP" =paintor_all_fun_res$SNP ,"PIP" = paintor_all_fun_res$Posterior_Prob,"CAUSAL"=ifelse(paintor_all_fun_res$Signal!=0,1,0)))
      ...
      set_res_all<-rbind(set_res_all,rbind(c(MESuSiE_SET,"MESuSiE"),c( SuSiE_SET,"SuSiE"),c( SuSiE_weighted_SET,"SuSiE_meta_weighted"),c( SuSiE_merged_SET,"SuSiE_meta_merged"),c(Paintor_SET$Paintor_set,"Paintor"),c(Paintor_fun_SET$Paintor_set,"Paintor_fun"),c(Paintor_all_fun_SET$Paintor_set,"Paintor_all_fun"),c(susiex_SET,"SuSiEx"),c(XMAP_SET,"XMAP"),c(multisusie_SET,"MultiSuSiE"),c(carmax_SET,"CARMAX")))
      ...
      Paintor_fun_ROC = Paintor_fun_ROC + compute_ROC(data.frame("SNP" =paintor_fun_res$SNP ,"PIP" = paintor_fun_res$Posterior_Prob),ifelse(paintor_fun_res$Signal==3,1,0))
      Paintor_all_fun_ROC = Paintor_all_fun_ROC + compute_ROC(data.frame("SNP" =paintor_all_fun_res$SNP ,"PIP" = paintor_all_fun_res$Posterior_Prob),ifelse(paintor_all_fun_res$Signal==3,1,0))
      ...
      Paintor_fun_ROC_Either = Paintor_fun_ROC_Either + compute_ROC(data.frame("SNP" =paintor_fun_res$SNP ,"PIP" = paintor_fun_res$Posterior_Prob),ifelse(paintor_fun_res$Signal!=0,1,0))
      Paintor_all_fun_ROC_Either = Paintor_all_fun_ROC_Either + compute_ROC(data.frame("SNP" =paintor_all_fun_res$SNP ,"PIP" = paintor_all_fun_res$Posterior_Prob),ifelse(paintor_all_fun_res$Signal!=0,1,0))
      ...
      Paintor_fun_ROC_EU = Paintor_fun_ROC_EU + compute_ROC(data.frame("SNP" =paintor_fun_res$SNP ,"PIP" = paintor_fun_res$Posterior_Prob),ifelse(paintor_fun_res$Signal==1,1,0))
      Paintor_fun_ROC_BB = Paintor_fun_ROC_BB + compute_ROC(data.frame("SNP" =paintor_fun_res$SNP ,"PIP" = paintor_fun_res$Posterior_Prob),ifelse(paintor_fun_res$Signal==2,1,0))

      Paintor_all_fun_ROC_EU = Paintor_all_fun_ROC_EU + compute_ROC(data.frame("SNP" =paintor_all_fun_res$SNP ,"PIP" = paintor_all_fun_res$Posterior_Prob),ifelse(paintor_all_fun_res$Signal==1,1,0))
      Paintor_all_fun_ROC_BB = Paintor_all_fun_ROC_BB + compute_ROC(data.frame("SNP" =paintor_all_fun_res$SNP ,"PIP" = paintor_all_fun_res$Posterior_Prob),ifelse(paintor_all_fun_res$Signal==2,1,0))
      ...
      ##Sumstat
      set_mat<-data.frame(matrix(0,ncol = 11,nrow = nrow(paintor_res)))
      colnames(set_mat)<-c("MESuSiE_cs","SuSiE_cs","SuSiE_weighted_cs","SuSiE_merged_cs","Paintor_cs","Paintor_fun_cs","Paintor_all_fun_cs","SuSiEx_cs","XMAP_cs","MultiSuSiE_cs","CARMAX_cs")
      ...
      set_mat$Paintor_fun_cs[which(paintor_fun_res$SNP%in%Paintor_fun_SET$Paintor_cs)]<-1
      set_mat$Paintor_all_fun_cs[which(paintor_all_fun_res$SNP%in%Paintor_all_fun_SET$Paintor_cs)]<-1
      ...
      locus_sumstat<-paintor_res%>%mutate(Paintor_cs = set_mat$Paintor_cs,Paintor_fun_cs = set_mat$Paintor_fun_cs,Paintor_fun_PIP = paintor_fun_res$Posterior_Prob,Paintor_all_fun_cs = set_mat$Paintor_all_fun_cs,Paintor_all_fun_PIP = paintor_all_fun_res$Posterior_Prob,
                                          CARMAX_cs = set_mat$CARMAX_cs,CARMAX_Either = carmax_pip_either, CARMAX_Shared = carmax_pip_shared, CARMAX_WB = carmax_pip_eur, CARMAX_BB = carmax_pip_afr,
    ...
    #######################################
    # Either signal part
    roc_data_list <- list(MESuSiE_ROC_Either = MESuSiE_ROC_Either, SuSiE_ROC_Either = SuSiE_ROC_Either,SuSiE_weighted_ROC_Either = SuSiE_weighted_ROC_Either,SuSiE_merged_ROC_Either = SuSiE_merged_ROC_Either,Paintor_ROC_Either = Paintor_ROC_Either,Paintor_fun_ROC_Either = Paintor_fun_ROC_Either,Paintor_all_fun_ROC_Either = Paintor_all_fun_ROC_Either,susiex_ROC_Either = susiex_ROC_Either,XMAP_ROC_Either = XMAP_ROC_Either, multisusie_ROC_Either = multisusie_ROC_Either, carmax_ROC_Either= carmax_ROC_Either)
    method_names <- c("MESuSiE", "SuSiE", "SuSiE_weighted", "SuSiE_merged", "Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")
    ...
    # Shared signal part
    roc_data_list <- list(MESuSiE_ROC = MESuSiE_ROC, SuSiE_ROC = SuSiE_ROC,  SuSiE_weighted_ROC = SuSiE_weighted_ROC,  SuSiE_merged_ROC = SuSiE_merged_ROC, Paintor_ROC = Paintor_ROC,Paintor_fun_ROC = Paintor_fun_ROC,Paintor_all_fun_ROC = Paintor_all_fun_ROC,susiex_ROC = susiex_ROC,XMAP_ROC = XMAP_ROC, multisusie_ROC = multisusie_ROC,carmax_ROC= carmax_ROC)
    method_names <- c("MESuSiE", "SuSiE", "SuSiE_weighted", "SuSiE_merged", "Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")
    ...
    roc_data_list <- list(MESuSiE_ROC_EU = MESuSiE_ROC_EU,MESuSiE_ROC_BB = MESuSiE_ROC_BB, SuSiE_ROC_EU = SuSiE_ROC_EU, SuSiE_ROC_BB = SuSiE_ROC_BB,SuSiE_weighted_ROC_EU = SuSiE_weighted_ROC_EU, SuSiE_weighted_ROC_BB = SuSiE_weighted_ROC_BB,SuSiE_merged_ROC_EU = SuSiE_merged_ROC_EU, SuSiE_merged_ROC_BB = SuSiE_merged_ROC_BB,Paintor_ROC_EU = Paintor_ROC_EU,Paintor_ROC_BB = Paintor_ROC_BB,Paintor_fun_ROC_EU = Paintor_fun_ROC_EU,Paintor_fun_ROC_BB = Paintor_fun_ROC_BB,Paintor_all_fun_ROC_EU = Paintor_all_fun_ROC_EU,Paintor_all_fun_ROC_BB = Paintor_all_fun_ROC_BB, susiex_ROC_EU = susiex_ROC_EU, susiex_ROC_BB = susiex_ROC_BB, XMAP_ROC_EU = XMAP_ROC_EU, XMAP_ROC_BB = XMAP_ROC_BB,multisusie_ROC_EU = multisusie_ROC_EU,multisusie_ROC_BB = multisusie_ROC_BB, carmax_ROC_EU =carmax_ROC_EU, carmax_ROC_BB = carmax_ROC_BB)
    method_names <- c("MESuSiE_EU", "MESuSiE_BB","SuSiE_EU", "SuSiE_BB","SuSiE_weighted_EU", "SuSiE_weighted_BB","SuSiE_merged_EU", "SuSiE_merged_BB","Paintor_EU", "Paintor_BB","Paintor_fun_EU","Paintor_fun_BB","Paintor_all_fun_EU","Paintor_all_fun_BB","SuSiEx_EU","SuSiEx_BB","XMAP_EU","XMAP_BB","MultiSuSiE_EU","MultiSuSiE_BB","CARMAX_EU","CARMAX_BB")
...
all_Set_data_dataframe<-all_Set_data_dataframe%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE","SuSiE_meta_weighted","SuSiE_meta_merged","Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")))
...
set_power_summary<-set_power_summary%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE", "SuSiE_meta_weighted","SuSiE_meta_merged","Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")))
...
PIP_calibration_either<-PIP_calibration_either%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Either","SuSiE" = "SuSiE_Either","SuSiE_meta_weighted" = "SuSiE_weighted_PIP","SuSiE_meta_merged" = "SuSiE_merged_PIP","Paintor" = "Paintor_PIP","Paintor_fun" = "Paintor_fun_PIP","Paintor_all_fun" = "Paintor_all_fun_PIP", "SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP","CARMAX" = "CARMAX_Either"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor","Paintor_fun","Paintor_all_fun", "SuSiEx","XMAP","MultiSuSiE","CARMAX"))
...
PIP_calibration_Shared<-PIP_calibration_Shared%>%mutate(Method = fct_recode(Method, "MESuSiE" = "MESuSiE_Shared","SuSiE" = "SuSiE_Shared","SuSiE_meta_weighted" = "SuSiE_weighted_PIP","SuSiE_meta_merged" = "SuSiE_merged_PIP","Paintor" = "Paintor_PIP","Paintor_fun" = "Paintor_fun_PIP","Paintor_all_fun" = "Paintor_all_fun_PIP","SuSiEx" = "SuSiEx_PIP","XMAP" = "XMAP_PIP","MultiSuSiE" = "MultiSuSiE_PIP","CARMAX" = "CARMAX_Shared"))%>%mutate(Method = fct_relevel(Method,"MESuSiE","SuSiE","Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX"))
...
data_all<-data_all%>%mutate(Paintor_fun_WB = Paintor_fun_PIP,Paintor_fun_BB = Paintor_fun_PIP)
data_all<-data_all%>%mutate(Paintor_all_fun_WB = Paintor_all_fun_PIP,Paintor_all_fun_BB = Paintor_all_fun_PIP)
...
PIP_calibration_WB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_WB,SuSiE_WB,SuSiE_weighted_WB,SuSiE_merged_WB,Paintor_WB,Paintor_fun_WB,Paintor_all_fun_WB,SuSiEx_WB, XMAP_WB, MultiSuSiE_WB, CARMAX_WB),c(1),c("MESuSiE_WB","SuSiE_WB","SuSiE_weighted_WB","SuSiE_merged_WB","Paintor_WB","Paintor_fun_WB","Paintor_all_fun_WB","SuSiEx_WB","XMAP_WB", "MultiSuSiE_WB","CARMAX_WB"))
PIP_calibration_BB<-create_obs_frq(data_all%>%select(Signal,h2,causal_num,MESuSiE_BB,SuSiE_BB,SuSiE_weighted_BB,SuSiE_merged_BB,Paintor_BB,Paintor_fun_BB,Paintor_all_fun_BB,SuSiEx_BB, XMAP_BB, MultiSuSiE_BB, CARMAX_BB),c(2),c("MESuSiE_BB","SuSiE_BB","SuSiE_weighted_BB","SuSiE_merged_BB","Paintor_BB","Paintor_fun_BB","Paintor_all_fun_BB","SuSiEx_BB","XMAP_BB", "MultiSuSiE_BB","CARMAX_BB"))

PIP_calibration_ancestry<-rbind(PIP_calibration_WB,PIP_calibration_BB)
PIP_calibration_ancestry<-PIP_calibration_ancestry%>%mutate(Method = fct_recode(Method, "MESuSiE_WB" = "MESuSiE_WB","MESuSiE_BB" = "MESuSiE_BB","SuSiE_WB" = "SuSiE_WB", "SuSiE_BB" = "SuSiE_BB","SuSiE_weighted_WB" = "SuSiE_weighted_WB","SuSiE_weighted_BB" = "SuSiE_weighted_BB","SuSiE_merged_WB" = "SuSiE_merged_WB","SuSiE_merged_BB" = "SuSiE_merged_BB","Paintor_WB" = "Paintor_WB","Paintor_BB" = "Paintor_BB","Paintor_fun_WB" = "Paintor_fun_WB","Paintor_fun_BB" = "Paintor_fun_BB","Paintor_all_fun_WB" = "Paintor_all_fun_WB","Paintor_all_fun_BB" = "Paintor_all_fun_BB","SuSiEx_WB" = "SuSiEx_WB","SuSiEx_BB" = "SuSiEx_BB","XMAP_WB" = "XMAP_WB","XMAP_BB" = "XMAP_BB", "MultiSuSiE_WB" = "MultiSuSiE_WB", "MultiSuSiE_BB" = "MultiSuSiE_BB","CARMAX_WB" = "CARMAX_WB","CARMAX_BB" = "CARMAX_BB"))%>%mutate(Method = fct_relevel(Method,"MESuSiE_WB","MESuSiE_BB","SuSiE_WB","SuSiE_BB","SuSiE_weighted_WB","SuSiE_weighted_BB","SuSiE_merged_WB","SuSiE_merged_BB","Paintor_WB", "Paintor_BB","Paintor_fun_WB","Paintor_fun_BB","Paintor_all_fun_WB","Paintor_all_fun_BB","SuSiEx_WB","SuSiEx_BB","XMAP_WB","XMAP_BB","MultiSuSiE_WB", "MultiSuSiE_BB","CARMAX_WB","CARMAX_BB"))
levels(PIP_calibration_ancestry$Method)<-c(paste0("MESuSiE~","WB"),paste0("MESuSiE~","BB"),paste0("SuSiE~","WB"),paste0("SuSiE~","BB"),paste0("SuSiE_weighted~","WB"),paste0("SuSiE_weighted~","BB"),paste0("SuSiE_merged~","WB"),paste0("SuSiE_merged~","BB"),paste0("Paintor~","WB"),paste0("Paintor~","BB"),paste0("Paintor_fun~","WB"),paste0("Paintor_fun~","BB"),paste0("Paintor_all_fun~","WB"),paste0("Paintor_all_fun~","BB"),paste0("SuSiEx~","WB"),paste0("SuSiEx~","BB"),paste0("XMAP~","WB"),paste0("XMAP~","BB"),paste0("MultiSuSiE~","WB"),paste0("MultiSuSiE~","BB"),paste0("CARMAX~","WB"),paste0("CARMAX~","BB"))
...
ancestry_all_ROC_data_dataframe<-ancestry_all_ROC_data_dataframe%>%mutate(Method = fct_recode(Method,
                                                                                              ...
                                                                                              "Paintor_fun BB" = "Paintor_fun_BB",
                                                                                              "Paintor_fun WB" = "Paintor_fun_EU",
                                                                                              "Paintor_all_fun BB" = "Paintor_all_fun_BB",
                                                                                              "Paintor_all_fun WB" = "Paintor_all_fun_EU",
                                                                                              "SuSiEx WB" = "SuSiEx_EU",
                                                                                              "SuSiEx BB" = "SuSiEx_BB",
                                                                                              "XMAP WB" = "XMAP_EU",
                                                                                              "XMAP BB" = "XMAP_BB",
...
FDR_Power_shared<-FDR_Power(shared_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE","SuSiE_weighted","SuSiE_merged","Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")))
FDR_Power_shared%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))


FDR_Power_either<-FDR_Power(either_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method, c("MESuSiE", "SuSiE","SuSiE_weighted","SuSiE_merged","Paintor","Paintor_fun","Paintor_all_fun","SuSiEx","XMAP","MultiSuSiE","CARMAX")))
FDR_Power_either%>%group_by(Method,FDR)%>%summarise(mean_power= mean(Power))

FDR_Power_ancestry<-FDR_Power(ancestry_all_ROC_data_dataframe)%>%mutate(Method = fct_relevel(Method,"MESuSiE BB","MESuSiE WB", "SuSiE BB" ,"SuSiE WB","SuSiE_weighted BB","SuSiE_weighted WB","SuSiE_merged WB","SuSiE_merged BB","Paintor BB" ,"Paintor WB","Paintor_fun BB","Paintor_fun WB","Paintor_all_fun BB","Paintor_all_fun WB","SuSiEx BB" ,"SuSiEx WB","XMAP WB","XMAP BB","MultiSuSiE WB","MultiSuSiE BB","CARMAX WB", "CARMAX BB"))
...
colnames(time_all)<-c("MESuSiE","SuSiE","Paintor","Paintor_fun","Paintor_all_fun","XMAP","SuSiEx","CARMAX","MultiSuSiE","NUM_SNP")
...
method_order_base <- c("MFD", "MESuSiE", "SuSiE", "SuSiE_weighted", "SuSiE_merged",
                       "Paintor", "Paintor_fun", "Paintor_all_fun", "MultiSuSiE", "SuSiEx", "XMAP", "CARMAX")
