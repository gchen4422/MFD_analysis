...
wrk_dir<-paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/polygenic_sensitivity/causal_num_",num_causal,"/")
...
start.time<-Sys.time()
MESuSiE_res<-meSuSie_core(R_mat_list,summary_stat_sd_list,L=20,residual_variance=NULL,prior_weights=NULL,ancestry_weight=ancestry_weight,optim_method ="optim",estimate_residual_variance =T,max_iter =100)
...
sst_file <- paste0("/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/polygenic_sensitivity/causal_num_", num_causal,
                   "/summary_data/CAUSAL_", num_causal, "_LOCI_", LD_BLOCK, "_h2_", h2_num, "_eur,/scratch/negishi/chen4422/hapnest/Simulation_update/simu_region_1mb/polygenic_sensitivity/causal_num_",
...
library(susieR)
start.time<-Sys.time()
susie_EU<-susie_rss(zfile$zscore_1,EU_cov,check_prior=F, n = 300000,L =20)
susie_BB<-susie_rss(zfile$zscore_2,BB_cov,check_prior=F, n = 300000,L =20)
...
carmax_results <- CARMAX(z.list, ld.list, lambda.list = lambda.list, effect.size.prior = prior.name, LD.estimation = FALSE, outlier.switch = FALSE, num.causal = 20)
