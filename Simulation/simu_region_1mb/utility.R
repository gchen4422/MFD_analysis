...
Set_Size_fun_order <- function(all_Set_data_dataframe, upper_limit) {
  ...
  method_order <- c("MESuSiE", "SuSiE", "SuSiE-weighted", "SuSiE-merged", 
                    "Paintor", "MultiSuSiE", "SuSiEx", "XMAP", "CARMAX")
  
  p_size_box <- ggplot(data = all_Set_data_dataframe, 
                       # 2. Use factor() with levels to enforce the order on x-axis
                       aes(x = factor(Method, levels = method_order), 
                           y = Size, 
                           fill = Method)) +
    geom_boxplot(aes(fill = Method), outlier.size = 0.1) +
    coord_cartesian(ylim = c(0, upper_limit)) +
    scale_fill_manual(values = c("MESuSiE"="#8da0cb", "SuSiE"="#66c2a5", 
                                 "SuSiE-weighted"="#B2D3A4", "SuSiE-merged"="#9FBA95", 
                                 "Paintor"="#fc8d62", "MultiSuSiE"="#e78ac3", 
                                 "SuSiEx"="#E89DA0", "XMAP"="#ffd92f", "CARMAX"="#f2b56e"), 
                      guide = "none") + # guide=FALSE is deprecated; use "none"
    ylab("Set Size") +
    xlab("") +
    facet_grid(h2 ~ causal_num, labeller = label_parsed, scales = "free", space = "free") +
    theme_bw() + 
...
Set_Power_fun_legend <- function(power_summary){
  p_power_bar = ggplot(data = power_summary, aes(x = Method, y = Power_name, fill = Method)) +
    geom_bar(stat = "identity", position = "dodge", aes(fill = Method), alpha = 1.2) +
  ...
  p_power_bar = p_power_bar + 
    geom_text(aes(x = Method, y = Power_name, label = round(Power_name, 2)), 
              position = position_dodge(width = 1), vjust = -0.5, size = 5 * 5 / 10)
...
ROC_ancestry_fun <- function(all_ROC_data_dataframe){
  ...
  p = ggplot(all_ROC_data_dataframe, aes(x = Power, y = 1 - FDR)) + 
    ...
    # 1. Main Lines
    geom_line(aes(linetype = Ancestry, color = Method), size = 0.8) +
    
    # 2. Points (PIP > 0.5)
    # Note: show.legend = c(color = FALSE) prevents these dots from ruining the line legend
    geom_point(data = all_ROC_data_dataframe %>% filter(Cutoff == 0.5), 
               aes(x = Power, y = 1 - FDR, group = Method, color = Method, shape = 'PIP > 0.5'), 
    ...
    # 3. Scales
    scale_color_manual(
      values = c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5",
                 "SuSiE_weighted"="#B2D3A4","SuSiE_merged"="#9FBA95",
                 "Paintor"="#fc8d62","MultiSuSiE"="#e78ac3",
                 "SuSiEx"="#E89DA0","XMAP"="#ffd92f",
                 "CARMAX"="#f2b56e")
    ) + 
    scale_linetype_manual(values = c("White British" = "dashed", "Black British" = "solid")) +
    ...
    guides(
      # Method: 9 items spread across 2 rows
      color = guide_legend(nrow = 2, order = 1),
      
      # Ancestry: Stack vertically (2 rows) to align neatly with Method
      linetype = guide_legend(nrow = 2, order = 2),
      
      # Shape: Just sits at the end
...
FDR_Power_shared_fun_noannot<-function(FDR_Power){
  p<-ggplot(FDR_Power, aes(FDR, Power, fill = Method))+
    geom_bar(stat="identity", position = "dodge")+
    #geom_text(aes(x=FDR,group=Method,y=Power,label=Power),position = position_dodge(width = 1),vjust=-0.5,size = 5*5/14)+
    scale_fill_manual(values=c("MESuSiE"="#8da0cb","SuSiE"="#66c2a5","SuSiE_weighted"="#B2D3A4","SuSiE_merged"="#9FBA95","Paintor"="#fc8d62","MultiSuSiE"="#e78ac3","SuSiEx"="#E89DA0","XMAP"="#ffd92f","CARMAX"="#f2b56e"),guide=FALSE)+
...
PIP_calibration_shared_byh2_fun<-function(PIP_calibration){
  ...
  p = p + scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))+scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))
  p = p +	custom_theme() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2)+ theme(axis.text.x = element_text(size = 5))
  return(p)
}
#######################################################
#
...
PIP_calibration_ancestry_fun<-function(PIP_calibration){
  ...
  p = p + scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))+scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),labels=c("0", "0.25", "0.5", "0.75", "1"))
  p = p +	custom_theme() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = -Inf, colour = "black", size = 0.2) + theme(strip.text.y = element_text(size = 5))
  return(p)
}
